...
import com.movtery.zalithlauncher.utils.platform.getMaxMemoryWithSwap
...
abstract class Launcher(
    ...
    private suspend fun launchJavaVM(
        ...
        val effectiveRamAllocation = if (AllSettings.autoRamAllocation.getValue()) {
            if (AllSettings.enableSwapMemory.getValue()) {
                // Swap/ZRAM ile RAM limitini aş - büyük modlar için
                getMaxMemoryWithSwap(context)
            } else {
    ...
    protected open fun progressFinalUserArgs(
        ...
        // Swap/ZRAM bellek optimizasyonu - RAM dolduğunda çöküşü önler
        // Fizik modu gibi yüksek bellek gerektiren modlar için gereklidir
        if (AllSettings.enableSwapMemory.getValue()) {
            ...
            args.add("-XX:SoftRefLRUPolicyMSPerMB=0")
        }
